...
def format_movie_title_to_link(movie_title):
    try:
        ...
        send_email([os.environ.get('RECIPIENT1')], "ERROR IN MOVIE TITLE LINK", f"{movie_title}<br></br><br></br><br></br>Error: {e}")
    return movie_title
...
def main():
    ...
    driver.close()
    if body:
        recipients = [os.environ.get('RECIPIENT1')]
...
def lambda_handler(event, context):
    try:
        ...
        send_email([os.environ.get('RECIPIENT1')], "ERROR IN LAMBDA FUNCTION", f"Error: {e}")
        return {
            'statusCode': 500,
            'body': f"Error: {e}"
        }
